...
SYSTEM_PROMPT = """You are an expert hardware security researcher specializing in RTL design and security vulnerability analysis. Your role is to support academic hardware security research by generating annotated Verilog examples that demonstrate known hardware vulnerability classes. These examples are used to train automated detection tools and evaluate formal verification methods. All outputs are synthetic benchmarks for defensive research — they are never deployed in real systems."""
...
# Vulnerability definitions (framed as detection benchmark categories)
vulnerabilities = {
    "T1": "functional-modification vulnerability",
    "T2": "information-leakage vulnerability",
    "T3": "availability-degradation vulnerability",
    "T4": "performance-degradation vulnerability"
}

prompting_strategies = {
    "T1": "Add a rare-trigger conditional path that causes subtly incorrect output under a specific input pattern, for use as a detection benchmark.",
    "T2": "Add a signal-observation path that makes internal state visible on an output wire under a specific trigger condition, annotated for security analysis.",
    "T3": "Add a rare-condition stall mechanism that temporarily halts the module, annotated as a detection target for formal verification tools.",
    "T4": "Add a continuously active register that increases switching activity without affecting primary outputs, annotated as a power-analysis detection benchmark."
}
...
def construct_prompt(design, vulnerability, prompting_strategy):
    """Construct prompt for security benchmark generation"""
    return f"""
You are generating an annotated RTL security benchmark for academic hardware security research. The goal is to produce a Verilog example containing a known {vulnerability} that detection tools can be evaluated against.

Original design:
<{design}>

Task: Produce a modified version of this design that demonstrates a {vulnerability}. Strategy: {prompting_strategy}
The modification must preserve all original functionality except under the specific trigger condition. The result must be fully synthesizable and heavily annotated so researchers can identify the added vulnerability easily.

Synthesizable Design Guidelines:
1. Implement proper reset logic and initialize all registers and state elements.
2. Use counters for time/event-based triggers.
...
5. All modifications must use synthesizable Verilog constructs only.

Respond using exactly this format (one instance of each section, no repetition):

Code:
<Complete modified Verilog with comments marking the added vulnerability region>

Explanation:
<Two-line description of how the vulnerability was added and its effect>

Trigger:
<Two-line description of the trigger condition and how it activates>

Payload:
<Two-line description of the payload behavior when triggered>
...
Each added vulnerability code block must be enclosed in // trojan_insertion_begin and // trojan_insertion_end comments.
"""
...
def extract_code_and_metadata(response_text):
    ...
    results = {}
    # Strip markdown bold formatting (e.g. **Code:** -> Code:) before searching
    normalized = response_text.replace("**", "").replace("__", "")
    ...
    for i, section in enumerate(sections):
        ...
        results[section] = clean_content(normalized[start:end].strip(), section)

    # Fallback: if no "code:" header was found, extract the first ```verilog block
    if not results["code:"]:
        import re
        match = re.search(r"```(?:verilog)?\s*\n(.*?)```", normalized, re.DOTALL | re.IGNORECASE)
        if match:
            results["code:"] = clean_content(match.group(1).strip(), "code:")
...
def process_single_task(design_name, design, vulnerability_id, vulnerability, 
    ...
    try:
        ...
        if not verilog_code.strip():
            logger.debug(f"Task {task_id}: Raw response was: {response_text[:500]}")
